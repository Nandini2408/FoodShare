...
export default function ActivityPage() {
  ...
  const { data: allPosts, isLoading: allPostsLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiRequest("GET", "/api/posts?limit=1000");
      return await res.json();
    },
    enabled: !!user,
  });
  ...
  const totalDonationsMade = allPosts?.filter((p: any) => p.type === "donation" && p.userId === user?.id).length || 0;
  const totalRequestsMade = allPosts?.filter((p: any) => p.type === "request" && p.userId === user?.id).length || 0;
  // For received, count posts where the user is a claimer (from myClaims)
        ...
        <Card className="border border-gray-200 dark:border-gray-700 shadow-md rounded-xl dark:bg-gray-800 transition-colors duration-300">
          ...
          <CardContent className="py-6">
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div className="flex flex-col items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 transition-colors duration-300">
                <span className="text-xs text-gray-500 dark:text-gray-400">Total Requests Made</span>
                <span className="text-2xl font-bold text-blue-600 dark:text-blue-400">{totalRequestsMade}</span>
              </div>
            </div>
            <div className="mb-6">
